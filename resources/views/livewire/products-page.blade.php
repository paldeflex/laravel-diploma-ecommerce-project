                        ...
                        <div class="p-4 mb-5 bg-white border border-gray-200 dark:border-gray-900 dark:bg-gray-900">
                            <div class="flex items-center justify-between">
                                ...
                                @if(count($selectedCategories))
                                    <a href="#" wire:click.prevent="resetCategoriesFilters"
                                       class="text-sm text-blue-500 font-bold mt-auto">
                                        Сброс
                                    </a>
                                @endif
                            </div>
                                ...
                                @forelse($categories->take($categoriesToShow) as $category)
                                    <li class="mb-4" wire:key="{{ $category->id }}">
                                        <label for="{{$category->slug}}"
                                               class="flex items-center dark:text-gray-400 cursor-pointer">
                        ...
                        <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                            <div class="flex items-center justify-between">
                                ...
                                @if(count($selectedCoatingTypes))
                                    <a href="#" wire:click.prevent="resetCoatingTypesFilters"
                                       class="text-sm text-blue-500 font-bold mt-auto">
                                        Сброс
                                    </a>
                                @endif
                            ...
                            <ul>
                                @forelse($coatingTypes->take($coatingTypesToShow) as $coatingType)
                                    <li class="mb-4" wire:key="{{ $coatingType->id }}">
                                        <label for="{{ $coatingType->slug }}"
                                               class="flex items-center dark:text-gray-300 cursor-pointer">
                                            <input type="checkbox" wire:model.live="selectedCoatingTypes"
                                                   wire:loading.delay.attr="disabled"
                                                   class="w-4 h-4 mr-2 min-w-[16px] min-h-[16px]"
                                                   id="{{ $coatingType->slug }}" value="{{ $coatingType->id }}">
                                            <span
                                                class="text-lg ml-1 dark:text-gray-400">{{ $coatingType->name }}</span>
                                        </label>
                                    </li>
                                @empty
                                ...
                                <li class="mb-4">
                                    <label for="featured" class="flex items-center dark:text-gray-300 cursor-pointer">
                                        <input type="checkbox" wire:model.live="featured" value="1"
                                               class="w-4 h-4 mr-2 min-w-[16px] min-h-[16px]" id="featured">
                                ...
                                <li class="mb-4">
                                    <label for="onSale" class="flex items-center dark:text-gray-300">
                                        <input type="checkbox" wire:model.live="onSale"
                                               class="w-4 h-4 mr-2 min-w-[16px] min-h-[16px]" id="onSale">
                        ...
                        <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                            ...
                            <div>
                                ...
                                <div class="flex justify-between ">
                                    <span
                                        class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency($minPrice, 'RUB', 'ru_RU') }}</span>
                                    <span
                                        class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency($maxPrice, 'RUB', 'ru_RU') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div>
                            <div class="flex flex-wrap -mx-3">
                                @forelse($products as $product)
                                    <a href="/products/{{ $product->slug }}" class="w-full sm:w-1/2 md:w-1/3 p-3"
                                       wire:key="{{ $product->id }}">
                                        <div
                                            class="flex flex-col h-full border border-gray-300 dark:border-gray-700 transition-transform duration-300 ease-in-out hover:-translate-y-1 hover:shadow-lg">
                                                ...
                                                <div class="mt-2">
                                                    @if($product->category)
                                                        <div class="text-sm text-gray-600 dark:text-gray-400">
                                                            Категория: <span
                                                                class="font-medium">{{ $product->category->name }}</span>
                                                        </div>
                                                    @endif
                                                    @if($product->coatingTypes->isNotEmpty())
                                                        <div class="flex flex-wrap mt-1">
                                                            @foreach($product->coatingTypes as $coatingType)
                                                                <span
                                                                    class="mr-2 mb-1 px-2 py-1 bg-gray-200 text-xs rounded dark:bg-gray-700 dark:text-gray-300">{{ $coatingType->name }}</span>
                                                            @endforeach
                                                        </div>
                                                    @endif
